...
async def handle_diag(request):
    ...
    # Stream download diagnostic test with full untruncated URLs
    all_streams = []
    try:
        ...
        all_streams = res_p.json().get("data", {}).get("streams", [])
    except Exception as ep:
        results["stream_fetch_err"] = str(ep)

    dl_tests = {}
    for idx, st in enumerate(all_streams):
        s_url = st.get("url")
        s_res = st.get("resolutions")
        s_key = f"stream_{idx}_{s_res}p"
        dl_tests[s_key] = {"url": s_url[:120]}

        # Test A: aiohttp with current downloader headers & ssl=False (the exact bot setting)
        try:
            import aiohttp
            conn_a = aiohttp.TCPConnector(ssl=False)
            curr_headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
                "Accept": "*/*",
            ...
            async with aiohttp.ClientSession(headers=curr_headers, connector=conn_a) as sa:
                async with sa.get(s_url, timeout=aiohttp.ClientTimeout(total=6)) as r_a:
                    dl_tests[s_key]["aiohttp_ssl_false_themoviebox"] = {
                        "status": r_a.status,
                        "content_length": r_a.headers.get("Content-Length"),
                        "body": (await r_a.text())[:150]
                    }
        except Exception as e:
            dl_tests[s_key]["aiohttp_ssl_false_themoviebox"] = {"error": str(e)}

        # Test B: httpx with clean aoneroom referer & geo headers
        try:
            h_b = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
                "Accept": "*/*",
            ...
            async with httpx.AsyncClient(verify=True, timeout=6.0) as sb:
                r_b = await sb.get(s_url, headers=h_b)
                dl_tests[s_key]["httpx_aoneroom_geo"] = {
                    "status": r_b.status_code,
                    "content_range": r_b.headers.get("content-range"),
                    "content_length": r_b.headers.get("content-length"),
                    "bytes_read": len(r_b.content),
                    "body": r_b.text[:150] if r_b.status_code != 206 else "SUCCESS"
                }
        except Exception as e:
            dl_tests[s_key]["httpx_aoneroom_geo"] = {"error": str(e)}

        # Test C: httpx with videodownloader.site referer & origin (ThrottleBuster default)
        try:
            h_c = {
            ...
            async with httpx.AsyncClient(verify=True, timeout=6.0) as sc:
                r_c = await sc.get(s_url, headers=h_c)
                dl_tests[s_key]["httpx_videodownloader_site"] = {
                    "status": r_c.status_code,
                    "content_range": r_c.headers.get("content-range"),
                    "content_length": r_c.headers.get("content-length"),
                    "bytes_read": len(r_c.content),
                    "body": r_c.text[:150] if r_c.status_code != 206 else "SUCCESS"
                }
        except Exception as e:
            dl_tests[s_key]["httpx_videodownloader_site"] = {"error": str(e)}

        # Test D: aiohttp with clean aoneroom referer & default ssl
        try:
            h_d = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
                "Accept": "*/*",
            ...
            conn_d = aiohttp.TCPConnector(ssl=True)
            async with aiohttp.ClientSession(headers=h_d, connector=conn_d) as sd:
                async with sd.get(s_url, timeout=aiohttp.ClientTimeout(total=6)) as r_d:
                    dl_tests[s_key]["aiohttp_clean_ssl_true"] = {
                        "status": r_d.status,
                        "content_range": r_d.headers.get("Content-Range"),
                        "content_length": r_d.headers.get("Content-Length"),
                        "body": (await r_d.text())[:150] if r_d.status != 206 else "SUCCESS"
                    }
        except Exception as e:
            dl_tests[s_key]["aiohttp_clean_ssl_true"] = {"error": str(e)}
